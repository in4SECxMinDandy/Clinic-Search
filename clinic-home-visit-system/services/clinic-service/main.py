"""
Clinic Service - Main Application
"""
import sys
...
from clinic_service.routers import clinics, doctors, gps
from clinic_service.utils.dependencies import set_database
...
async def lifespan(app: FastAPI):
    global db, redis
    logger.info("clinic_service_starting", version=settings.APP_VERSION)
    ...
    logger.info("clinic_service_shutdown")


app = FastAPI(
    title=settings.APP_NAME,
    version=settings.APP_VERSION,
    description="Clinic Management Service with GPS Features",
    lifespan=lifespan,
)
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "clinic-service", "version": settings.APP_VERSION}
